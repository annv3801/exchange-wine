...
function CustomLink({ to, children, onClick }) {
    ...
    var path = to === '/trang-chu'? '/' : `/category${to}`
    return (
        <Link href={path}>
            <a onClick={onClick} className={`hover:text-[#007bff] text-gray-600 font-semibold text-[15px] block ${router.pathname === to ? 'active-link' : 'inactive-link'}`}>
                {children}
            </a>
        </Link>
    );
        ...
        <header ref={node} className='shadow-md bg-white font-[sans-serif] sticky top-0 z-50'>
            ...
            <div className="bg-[#0090d0]">
                <div className='flex lg:justify-between relative py-3 sm:px-10 px-4 border-gray-200 border-b min-h-[75px] container mx-auto'>
                    <a href="" className="my-auto"><img src="/icon/Screenshot_5.png" alt="logo"
                                                         className='h-10 '/>
                    </a>
                    ...
                    <div className="lg:flex items-center hidden">
                        <a href={`tel:${configData?.phoneNumber}`} className="relative flex text-white bg-[#004275] rounded-3xl">
            ...
            <div className='flex flex-wrap justify-center px-10 lg:py-3 relative z-10'>
                <ul className={`lg:flex z-50 lg:translate-x-0 lg:relative lg:w-auto lg:overflow-visible lg:bg-transparent lg:space-y-0 max-lg:space-y-3 ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'} px-5 py-6 md:px-0 md:py-0 fixed inset-y-0 left-0 w-4/5 transform transition-transform duration-200 ease-in-out bg-white h-full overflow-auto`}>
                                                ...
                                                (<div onClick={toggleDropdown}
                                                    ...
                                                    {isDropdownOpen && (<ul
                                                            ...
                                                            {subMenuItems.map((subItem, j) =>
                                                                <li key={j}
                                                                    className='md:bg-white md:rounded-none p-2 md:p-0 border-b md:py-3'>
                                                                    <CustomLink
                                                                        to={`/${subItem.slug}`}
                                                                        onClick={closeMenu}>
                                                                        {subItem.name}
                    ...
                    <div className="md:hidden absolute bottom-4 left-4">
                        <a href={`tel:${configData?.phoneNumber}`} className="relative hover:text-blue-500 flex gap-5">
